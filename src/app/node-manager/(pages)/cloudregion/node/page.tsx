...
const Node = () => {
  ...
  const [expandedDataMapping, setExpandedDataMapping] = useState<
    Record<string, NodeExpanddata[]>
  >({});
        ...
        {data.map((item: NodeExpanddata) => {
          ...
          return (
            <div key={item.key} className="flex h-[18px]">
              <p className="ml-8 h-full leading-[18px] text-center">
                {item.name}
              </p>
              ...
              <div className="flex items-center ml-8 h-full">
                <Button
                  ...
                  onClick={() => {
                    ...
                    router.push(
                      `/node-manager/cloudregion/configuration?id=${item.key}&name=${name}&cloud_region_id=${cloud_region_id}`
                    );
                  }}
                >
                  {item.filename || '--'}
    ...
    try {
      ...
      results.forEach(({ key, result }) => {
        if (result && Array.isArray(result.status?.collectors)) {
          const expandedData = result.status.collectors.map(
            (item: CollectorItem) => ({
              nodeid: key,
              key: item.configuration_id,
              name: item.collector_name,
  ...
  const handleCollectorSuccess = async () => {
    ...
    res.forEach((resitem: any) => {
      if (expandedRowKeys.includes(resitem.id)) {
        const data =
          resitem?.status.collectors.map((item: CollectorItem) => ({
            nodeid: resitem.id,
            key: item.configuration_id,
            name: item.collector_name,
            filename: item.configuration_name,
